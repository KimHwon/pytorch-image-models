import random
import torch
from timm.models.quantization import quan_Conv2d, quan_Linear
import operator
from timm.attack.data_conversion import *
from contextlib import suppress
import logging

_logger = logging.getLogger(f'BFA')

class BFA(object):
    def __init__(self, criterion, model, k_top=10, gpu=None, logger=None):
        self.gpu = gpu
        if logger:
            _logger = logger
    ...
    def flip_bit(self, m):
        with suppress():
            ...
            w_grad_topk, w_idx_topk = m.weight.grad.detach().abs().contiguous().view(-1).topk(k_top)
            ...
            _, b_grad_max_idx = b_grad_topk.abs().contiguous().view(-1).topk(self.n_bits2flip)
            bit2flip = b_grad_topk.clone().contiguous().view(-1).zero_()

            if grad_max.item() != 0:  # ensure the max grad is not zero
                bit2flip[b_grad_max_idx] = 1
                bit2flip = bit2flip.contiguous().view(b_grad_topk.size())
            else:
                pass
            ...
            w_bin_topk_flipped = (bit2flip.short() * m.b_w.abs().short()).sum(0, dtype=torch.int16).sum(0, dtype=torch.int16) \
                    ^ w_bin_topk
            ...
            param_flipped = bin2int(w_bin,
                                    m.N_bits).contiguous().view(m.weight.data.size()).float()
        ...
        with suppress():
            ...
            for module_idx, (name, module) in enumerate(model.named_modules()):
                if name == max_loss_module:
                    ...
                    _logger.info(f'attacked module: {max_loss_module}')
                    ...
                    for i in range(attack_weight_idx.size()[0]):
                        ...
                        _logger.info(f'attacked weight index: {weight_idx}')
                        _logger.info(f'weight before attack: {weight_prior}')
                        _logger.info(f'weight after attack: {weight_post}')
    ...
    def random_flip_one_bit(self, model):
        ...
        for name, m in model.named_modules():
            if name == chosen_module:
                flatten_weight = m.weight.detach().contiguous().view(-1)
                ...
                _logger.info(f'attacked module: {chosen_module}')
                ...
                m.weight.data = flatten_weight.contiguous().view(m.weight.data.size())
