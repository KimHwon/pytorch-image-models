...
default_cfgs = {
    'tnt_quan_s_patch16_224': _cfg(
        url='https://github.com/contrastive/pytorch-image-models/releases/download/TNT/tnt_s_patch16_224.pth.tar',
        mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5),
    ),
    'tnt_quan_b_patch16_224': _cfg(
        mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5),
    ),
}
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, hidden_dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.qk = quan_Linear(dim, hidden_dim * 2, bias=qkv_bias)
        self.v = quan_Linear(dim, dim, bias=qkv_bias)
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.proj = quan_Linear(in_dim * num_pixel, dim, bias=True)
        # Outer transformer
...
class PixelEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, in_dim=48, stride=4):
        ...
        self.proj = quan_Conv2d(in_chans, self.in_dim, kernel_size=7, padding=3, stride=stride)
...
class TNT(nn.Module):
    ...
    def __init__(
        ...
        self.proj = quan_Linear(num_pixel * in_dim, embed_dim)
        ...
        self.head = quan_Linear(embed_dim, num_classes) if num_classes > 0 else nn.Identity()
...
@register_model
def tnt_quan_s_patch16_224(pretrained=False, **kwargs):
    model_cfg = dict(
        patch_size=16, embed_dim=384, in_dim=24, depth=12, num_heads=6, in_num_head=4,
        qkv_bias=False, **kwargs)
    model = _create_tnt('tnt_quan_s_patch16_224', pretrained=pretrained, **model_cfg)
    return model


@register_model
def tnt_quan_b_patch16_224(pretrained=False, **kwargs):
    model_cfg = dict(
        patch_size=16, embed_dim=640, in_dim=40, depth=12, num_heads=10, in_num_head=4,
        qkv_bias=False, **kwargs)
    model = _create_tnt('tnt_quan_b_patch16_224', pretrained=pretrained, **model_cfg)
    return model
